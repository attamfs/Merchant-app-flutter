      ...
      if (user != null) {
        _settingsSub = FirebaseFirestore.instance.collection('settings').doc('session').snapshots().listen((snap) {
          if (snap.exists && snap.data() != null) {
            ...
            int ms = 0;
            if (data.containsKey('merchantAppTimeoutValue') && data.containsKey('merchantAppTimeUnit')) {
              final val = int.tryParse(data['merchantAppTimeoutValue'].toString()) ?? 0;
              final unit = data['merchantAppTimeUnit'].toString().toLowerCase();
              ...
              final val = int.tryParse(data['merchantAppTimeoutMinutes'].toString()) ?? 0;
              ms = val * 60 * 1000;
            }
